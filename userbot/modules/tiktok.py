...
@register(outgoing=True, pattern=r"^\.ttaud(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return
    d_link = event.pattern_match.group(1)
    if "vm." and ".com" not in d_link:
        ...
        await event.edit("```Audio Sedang Diproses...```")
    chat = "@ttaudiobot"
    async with bot.conversation(chat) as conv:
        try:
            msg_start = await conv.send_message("/start")
            ...
            await event.edit(f"#Kesalahan ❌\n`Mohon Buka Blokir` @ttaudiobot `Lalu Tekan /start Dan Coba Lagi !`")
            return
        await bot.send_file(event.chat_id, audio, voice_note=True)
        await event.client.delete_messages(conv.chat_id,
                                           [msg_start.id, r.id, msg.id, details.id, audio.id])
        await event.delete()
