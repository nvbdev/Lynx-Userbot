...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**Maaf {ALIVE_NAME} Sedang ⚡𝙊𝙁𝙁𝙇𝙄𝙉𝙀⚡**",
...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**👤 {ALIVE_NAME} Telah ⚡𝙊𝙁𝙁𝙇𝙄𝙉𝙀⚡**\
        \n╰► **Alasan OFF :** `{string}`")
    else:
        await afk_e.edit(f"**👤 {ALIVE_NAME} Telah ⚡𝙊𝙁𝙁𝙇𝙄𝙉𝙀⚡**")
    if user.last_name:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name=user.last_name + "⚡𝙊𝙁𝙁𝙇𝙄𝙉𝙀⚡"))
    else:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name="⚡𝙊𝙁𝙁𝙇𝙄𝙉𝙀⚡"))
    if BOTLOG:
        await afk_e.client.send_message(BOTLOG_CHATID, f"#UPDATE\n{ALIVE_NAME} Telah ⚡𝙊𝙁𝙁𝙇𝙄𝙉𝙀⚡!")
    ISAFK = True
...
async def type_afk_is_not_true(notafk):
    ...
    user = await bot.get_me()  # pylint:disable=E0602
    last = user.last_name
    if last and last.endswith("⚡𝙊𝙁𝙁𝙇𝙄𝙉𝙀⚡"):
        last1 = last[:-12]
    else:
        last1 = ""
    ...
    if ISAFK:
        ...
        if BOTLOG:
            await notafk.client.send_message(
                BOTLOG_CHATID,
                "#INFO\nAnda Mendapatkan" + str(COUNT_MSG) + "Pesan.\nDari" +
                str(len(USERS)) + "Obrolan.\nSaat Anda Sedang ⚡𝙊𝙁𝙁𝙇𝙄𝙉𝙀⚡",
            )
            for i in USERS:
        ...
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"☆ {ALIVE_NAME} Masih ⚡𝙊𝙁𝙁𝙇𝙄𝙉𝙀⚡\n{afk_since} Yang Lalu.\n**Noted** : __Bila Anda Melakukan Spam Pada Saya, Anda Akan Terkena Global Banned Secara Otomatis.__ **Terimakasih.**\
                            \n╰► **Alasan OFF :** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"{ALIVE_NAME} Sedang ⚡𝙊𝙁𝙁𝙇𝙄𝙉𝙀⚡\n{afk_since} Yang Lalu.\n**Noted** : __Bila Anda Melakukan Spam Pada Saya, Anda Akan Terkena Global Banned Secara Otomatis.__ **Terimakasih.**\
                        \n╰► **Alasan OFF :** `{AFKREASON}`")
                else:
